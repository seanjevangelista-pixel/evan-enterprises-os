...
async function handle_welcome_email(req, res) {
  ...
  // businessName/ownerName/ownerEmail come straight from the request body on this
  // unauthenticated endpoint — escaped before landing in the HTML below.
  const businessNameSafe = escHtml(businessName);
  const ownerNameSafe    = escHtml(ownerName);
  const ownerEmailSafe   = escHtml(ownerEmail);
  const greeting    = ownerName ? `Hi ${escHtml(ownerName.split(' ')[0])},` : 'Hi there,';
...
<div style="max-width:560px;margin:0 auto">
  ...
  <div style="background:#fff;padding:28px;border:1px solid #E2E8F0;border-top:none">
    <div style="font-size:20px;font-weight:700;color:#0F172A;margin-bottom:16px">Welcome, ${businessNameSafe} 👋</div>
...
<div style="max-width:540px;margin:0 auto">
  ...
  <div style="background:#fff;padding:24px;border:1px solid #E2E8F0;border-top:none">
    <div style="font-size:16px;font-weight:700;margin-bottom:4px">New client added: ${businessNameSafe}</div>
    <div style="font-size:12px;color:#64748B;margin-bottom:20px">${ownerNameSafe} · ${ownerEmailSafe}</div>
    ...
    ${[
      ...
      ['Send welcome packet', `Already sent to ${ownerEmailSafe} ✓`],
  ...
  <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-top:none;border-radius:0 0 12px 12px;padding:12px 24px;font-size:11px;color:#94A3B8;text-align:center">
    Auto-generated by EVAN when ${businessNameSafe} was added to your dashboard
  </div>
</div>
</body></html>`;
...
async function handle_review_request(req, res) {
  ...
  // customerName/jobDescription/googleReviewLink are attacker-reachable — this
  // endpoint accepts unauthenticated POSTs — so they're escaped before landing in
  // the HTML below (reviewLink especially: unescaped, a value containing a
  // double-quote breaks out of the href attribute).
  const firstNameSafe      = escHtml(firstName);
  const jobDescriptionSafe = escHtml(jobDescription);
  const reviewLinkSafe     = escHtml(reviewLink);
...
<div style="max-width:560px;margin:0 auto">
  ...
  <div style="background:#fff;padding:32px 28px;border:1px solid #E2E8F0;border-top:none">
    ...
    <div style="font-size:13px;color:#374151;line-height:1.8;margin-bottom:28px">
      Hi ${firstNameSafe},<br><br>
      Thank you so much for choosing ${businessName}${jobDescriptionSafe ? ` for ${jobDescriptionSafe}` : ''}. It was a pleasure working with you, and we truly appreciate your business.<br><br>
    ...
    <div style="text-align:center;margin-bottom:28px">
      <a href="${reviewLinkSafe}" style="display:inline-block;background:#1D4ED8;color:#fff;font-size:15px;font-weight:700;padding:16px 36px;border-radius:10px;text-decoration:none;letter-spacing:0.01em">
    ...
    <div style="margin-top:24px;padding-top:20px;border-top:1px solid #F1F5F9;font-size:13px;color:#374151;line-height:1.7">
      Thank you again, ${firstNameSafe}. We hope to work with you again soon!<br><br>
...
async function handle_proposal(req, res) {
  ...
  // prospectName/city/niche/notes are attacker-reachable — this endpoint accepts
  // unauthenticated POSTs — so they're escaped before landing in the HTML below.
  const prospectNameSafe = escHtml(prospectName);
  const citySafe          = escHtml(city);
  const nicheSafe         = escHtml(niche);
  const notesSafe         = escHtml(notes);
  ...
  // An unrecognized service key falls back to `{ label: s }` — i.e. the raw,
  // attacker-supplied string from the `services[]` array lands in s.label — so
  // it's escaped here too, same reasoning as prospectName/city/niche/notes above.
  const serviceRows = selectedServices.map(s => `
    <tr>
      <td style="padding:14px 0;border-bottom:1px solid #F1F5F9;vertical-align:top">
        <div style="font-size:13px;font-weight:600;color:#0F172A;margin-bottom:3px">${escHtml(s.label)}</div>
  ...
  <div style="background:#fff;padding:32px;border:1px solid #E2E8F0;border-top:none">
    ...
    <div style="display:flex;justify-content:space-between;flex-wrap:wrap;gap:16px;margin-bottom:28px;padding-bottom:24px;border-bottom:1px solid #F1F5F9">
      <div>
        ...
        <div style="font-size:16px;font-weight:700;color:#0F172A">${prospectNameSafe}</div>
        <div style="font-size:12px;color:#64748B">${citySafe} ${nicheSafe ? '· ' + nicheSafe : ''}</div>
      </div>
      <div style="text-align:right">
    ...
    <div style="font-size:13px;color:#374151;line-height:1.8;margin-bottom:24px">
      ...
      Based on our conversation, here's what we recommend for <strong>${prospectNameSafe}</strong>:
    </div>
    ...
    <div style="background:#F8FAFC;border:1px solid #E2E8F0;border-radius:8px;padding:16px 20px;margin-bottom:24px">
      ...
      <div style="font-size:13px;color:#374151;line-height:1.6">${notesSafe}</div>
    </div>` : ''}
